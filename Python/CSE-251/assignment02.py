...
class shape_drawer(threading.Thread):
    def __init__(self,tur,shape):
    ...
    def run(self):
        if (self.shape == 'square'):
            draw_squares(self.tur)
        elif (self.shape == 'circle'):
            draw_circles(self.tur)
        elif (self.shape == 'rectangle'):
            draw_rectangles(self.tur)
        elif(self.shape == 'triangle'):
            draw_triangles(self.tur)
        else:
            print(f'error: {self.shape} is not an acceptable shape')
...
def run_with_threads(tur, log, main_turtle):
    ...
    #I like this idea I got to put everything in an array to make running
    #them start and join in less lines of code.
    threads = []
    '''
    origenal appempt -- will try again later

    #thread for squares
    square = threading.Thread(target=(draw_squares), args=())
    threads.append(square)
    #thread for circles
    circle = threading.Thread(target=(draw_circles), args=(tur))
    threads.append(circle)
    #thread for rectangles
    rectangle = threading.Thread(target=(draw_rectangles), args=(tur))
    threads.append(rectangle)
    #thread for triangles
    triangle = threading.Thread(target=(draw_triangles), args=(tur))
    threads.append(triangle)
    '''
    square = shape_drawer(tur,'square')
    circle = shape_drawer(tur, 'circle')
    rectangle = shape_drawer(tur,'rectangle')
    triangle = shape_drawer(tur, 'triangle')
    circle.start()
    circle.join()
    square.start()
    square.join()
