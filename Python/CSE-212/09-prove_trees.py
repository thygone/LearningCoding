...
class BST:
    ...
    def _contains(self, data, node):
        ...
        """
        if data == node.data: # if the values are the same then return true
            return True

        if node.right is not None and data > node.data: ## if left exists, tests it
            return self._contains(data, node.right) 
        elif node.left is not None and data < node.data: #if right exists test it.
...
def _insert_middle(sorted_list, first, last, bst):
    ...
    middle = int((last/2))
    #if middle == 0 then insert it
    
    bst.insert(sorted_list[middle])
    bst.insert(sorted_list[middle-1])
    bst.insert(sorted_list[middle+1])
