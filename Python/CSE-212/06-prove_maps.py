...
class Translator:
    ...
    def add_word(self, from_word, to_word):
        ...
        """
        ## plug in the words to a key with the same name as the english word. then make a dic of two
        ## keys for the english word and it's counterpart
        self.words[from_word] = {"eng" : from_word, "other" : to_word} ## other is the key to represent the other language
                                    ## eng stands for english word
    ...
    def translate(self, from_word):
        ...
        if from_word in self.words:
            return(self.words[from_word]["other"])
        else:
            return("???") ## if nothing is there then print this
...
def summarize_degrees(filename):
    ...
    file.
    """
    ## count all of the degrees of each type.
    i = 0
    degrees = dict() 
    with open(filename) as file_in:
        for line in file_in:
            fields = line.split(",") 
            if fields[3] in degrees: ## does the degree exist
                degrees[fields[3]] += 1 ## ues, so just increase the number by one
            else:
                degrees[fields[3]] = 1 ## no then add it to the list
...
# Sample Test Cases (may not be comprehensive) 
print("\n=========== PROBLEM 2 TESTS ===========")
print(summarize_degrees("cse212_programing_with_data_structures\\assignments\\w06\\census.txt")) # You might need to add a path for the file
...
def is_anagram(word1, word2):
    ...
    for lett in word1:
        if lett != " ": ## ignore spaces
            lett = lett.lower() ## make it all the same case
            if lett not in letters: 
                letters[lett ] = 1  ## this will make each letter a key that holds a count variable
            else: 
                letters[lett] += 1 ## incrament it
    
    for let in word2:
        if let != " ": ## ignore spaces
            let = let.lower() ## make it all the same case
            if let not in letters: ## if it does not exist end loop
                return(False)
            elif letters[let] == 0: ## is the count equal to 0 left?
                return(False)
            else: ## if it does remove the one letter from the list
                letters[let] -= 1 ## remove it
    
    for lleva in letters: ## check if there are any left over letters
        if letters[lleva] > 0:
            return(False)
    return(True)
...
class Maze:
    ...
    def move_left(self):
        ...
        """
        ## make the pointseasier to use
        x = self.curr_x
        y = self.curr_y
        if self.maze_map[x,y][0] == False: ## if left is false
    ...
    def move_right(self):
        ...
        x = self.curr_x
        y = self.curr_y       
        if self.maze_map[x,y][1] == False: ## if right is false
            print("Can't go that way!")
        else:
            self.curr_x += 1
    ...
    def move_up(self):
        ...
        x = self.curr_x
        y = self.curr_y   
        if self.maze_map[x,y][2] == False: ## if up is false
            print("Can't go that way!")
        else:
            self.curr_y -= 1 ## why does going up decrease the number??? this lead to a head ache Xp
    ...
    def move_down(self):
        ...
        x = self.curr_x
        y = self.curr_y       
        if self.maze_map[x,y][3] == False: ## if down is false
            print("Can't go that way!")
        
        else:
            self.curr_y += 1 ## for some reason going down increases the number... odd
...
def earthquake_daily_summary():
    ...
    ##places and magnatude
    quates = dict()
    for i in data:
        quates["features"] = data["features"]
    ## data holds the key of features that is a dictionary
    print(quates["features"])
    # ADD YOUR CODE HERE
